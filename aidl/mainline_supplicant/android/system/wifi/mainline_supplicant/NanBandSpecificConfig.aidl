...
parcelable NanBandSpecificConfig {
    /**
     * RSSI values controlling clustering behavior as per the spec. RSSI values are specified
     * without a sign, e.g. a value of -65dBm would be specified as 65.
     */
    byte rssiClose;
    ...
    /**
     * RSSI values determining whether discovery is near (used if enabled in discovery by
     ...
     * The publish/subscribe period values don't override the device level configurations if they
     * are specified.
     * Configuration is used only if |validDiscoveryWindowIntervalVal| is set to true.
