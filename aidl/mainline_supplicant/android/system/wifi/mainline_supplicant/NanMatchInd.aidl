...
parcelable NanMatchInd {
     ...
     * discovery. Matches the |NanDiscoveryCommonConfig.txMatchFilter| of the peer's
     * Unsolicited publish message or of the local device's Active subscribe message.
    ...
    /**
     * Indicates the type of discovery: true if match occurred on a Beacon frame, false if the
     * match occurred on a Service Discovery Frame (SDF).
     */
    boolean matchOccurredInBeaconFlag;

    /**
     * Flag to indicate that the firmware is out of resources and that it can no longer track this
     * Service Name. Indicates that while |ISupplicantNanIfaceEventCallback.eventMatch| will be
     * received, the |NanDiscoveryCommonConfig.discoveryMatchIndicator| configuration will not be
     * honored.
     */
    boolean outOfResourceFlag;

    /**
     * If RSSI filtering was enabled using |NanDiscoveryCommonConfig.useRssiThreshold| in the
     * discovery session setup, then this field contains the received RSSI value. It will contain 0
     * if RSSI filtering was not enabled.
     * RSSI values are returned without sign, e.g. -70dBm will be returned as 70.
     */
    byte rssiValue;

    /**
     * One of |NanCipherSuiteType| indicating the cipher type for data-paths constructed in the
    ...
    /**
     * Indicates whether the peer requires security enabled in any data-path (NDP) constructed in
     * the context of this discovery session. The |cipherType| specifies the cipher type for such
     * data-paths.
    ...
    /**
     * Indicates whether the peer requires (and hence allows) ranging in the context of this
     * discovery session. Note that ranging is only performed if all other match criteria with the
     * peer are met.
     ...
     * Ex: If NanMatchAlg is ONCE, but ranging indication is continuous, then continuous
     * match notifications will be received (with ranging information).
     * Ranging indication data is provided if the ranging required is enabled in the discovery
     * specification and:
     *   1) continuous ranging is specified.
     *   2) ingress/egress is specified and:
     *       - notify once for ingress >= ingress_distance and
     *         egress <= egress_distance,
     *       - same for ingress_egress_both
     * If the Awake DW intervals are larger than the ranging intervals, then priority is given to
     * the device DW intervals.
     * If ranging was required and executed, this contains the distance to the peer in mm.
     * The |rangingIndicationType| field specifies the event which triggered ranging.
     */
