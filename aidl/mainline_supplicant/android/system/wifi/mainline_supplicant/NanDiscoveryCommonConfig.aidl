...
parcelable NanDiscoveryCommonConfig {
    /**
     * NAN Match indication type. Controls how often to trigger
     * |ISupplicantNanIfaceEventCallback.eventMatch| for a single discovery session - i.e.
     * continuously discovering the same publisher with no new data.
     */
    @Backing(type="int")
    enum NanMatchAlg {
        ONCE = 0,
     ...
     * |ISupplicantNanIfaceEventCallback.notifyStartPublishResponse| or
     ...
     * canceled using |ISupplicantNanIface.stopPublishRequest| or
     ...
     * DW Interval are: 1, 2, 4, 8 and 16. A value of 0 will default to 1.
     * Does not override |NanBandSpecificConfig.discoveryWindowIntervalVal| configurations if those
     * are specified.
     */
    char discoveryWindowPeriod;

    /**
     * The lifetime of the discovery session in number of transmitted SDF discovery packets. A
     * value of 0 means forever or until canceled using |ISupplicantNanIface.stopPublishRequest| or
     * |ISupplicantNanIface.stopSubscribeRequest|.
     */
    byte discoveryCount;
     ...
     * values (A-Z, a-z, 0-9), the hyphen ('-'), and the period ('.').
     * All valid multi-byte UTF-8 characters are acceptable in a Service Name.
     */
    byte[] serviceName;

    /**
     * Specifies how often to trigger |ISupplicantNanIfaceEventCallback.eventMatch| when
     * continuously discovering the same discovery session (with no changes).
     */
     ...
     * This info is using Generic Service Protocol with setting Service Info type to 2 (Generic).
     ...
     * Max length: |NanCapabilities.maxMatchFilterLen|.
     * NAN Spec: matching_filter_tx and Service Descriptor
     * Attribute (SDA) / Matching Filter
     */
    byte[] txMatchFilter;

    /**
     * Specifies whether the discovery session uses the |NanBandSpecificConfig.rssiCloseProximity|
     * value (configured in enable/configure requests) to filter out matched discovered peers.
     * NAN Spec: Service Descriptor Attribute / Service Control / Discovery Range Limited.
     */
    boolean useRssiThreshold;

    /**
     * Controls whether the |ISupplicantNanIfaceEventCallback.eventPublishTerminated| (for publish
     * discovery sessions) or |ISupplicantNanIfaceEventCallback.eventSubscribeTerminated| (for
     * subscribe discovery sessions) will be delivered.
     */
    boolean disableDiscoveryTerminationIndication;

    /**
     * Controls whether |ISupplicantNanIfaceEventCallback.eventMatchExpired| will be delivered.
     */
    boolean disableMatchExpirationIndication;

    /**
     * Controls whether |ISupplicantNanIfaceEventCallback.eventFollowupReceived| will be delivered.
     */
    ...
    /**
     * Specifies whether there is a ranging requirement in this discovery session.
     * Ranging is only performed if all other match criteria with the peer are met. Ranging must be
     * performed if both peers in the discovery session (publisher and subscriber) set this flag to
     * true. Otherwise, if either peer sets this flag to false, ranging must not be performed and
     * must not impact discovery decisions.
    ...
    /**
     * Bitmap of |NanRangingIndication| values indicating the type of ranging feedback to be
     * provided by discovery session matches in |ISupplicantNanIfaceEventCallback.eventMatch|.
     * Only relevant if |rangingRequired| is true.
     */
    int configRangingIndications;

    /**
     * The ingress and egress distance in cm. If ranging is enabled (|rangingEnabled| is true)
     * then |configRangingIndications| is used to determine whether ingress and/or egress (or
     * neither) are used to determine whether a match has occurred.
